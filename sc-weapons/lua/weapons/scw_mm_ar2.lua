...
SWEP.Spawnable = true
SWEP.UseHands = true
SWEP.ViewModel = "models/weapons/c_scw_mm_ar2.mdl"
SWEP.ViewModelFOV = 54
SWEP.Weight = 5
SWEP.WorldModel = "models/weapons/w_scw_mm_ar2.mdl"
...
SWEP.Primary.CFG_Damage = ConVarExists("sk_plr_dmg_ar2") and GetConVar("sk_plr_dmg_ar2"):GetInt() * 2 or 16
SWEP.Primary.CFG_Delay = 0.075 -- 0.1
SWEP.Primary.CFG_Force = 4
SWEP.Primary.CFG_MaxVerticalKick = 1
SWEP.Primary.CFG_MinSpread = 0.001
SWEP.Primary.CFG_MaxSpread = 0.05
SWEP.Primary.CFG_SpreadRampStart = 25
SWEP.Primary.CFG_SpreadRampEnd = 75
SWEP.Primary.CFG_NPCSpread = Vector(0.05234, 0.05234, 0)
SWEP.Primary.CFG_Sound = "SCW.MM.AR2.Single"
SWEP.Primary.CFG_SoundNPC = "SCW.MM.AR2.Single_NPC"
-- SWEP Secondary Fire
SWEP.Secondary.Ammo = "AR2AltFire"
SWEP.Secondary.Automatic = true
SWEP.Secondary.ClipSize = -1
SWEP.Secondary.DefaultClip = -1
SWEP.Secondary.CFG_ChargeDelay = 0.5
SWEP.Secondary.CFG_Delay = 1
SWEP.Secondary.CFG_Force = 2000 -- 1000
SWEP.Secondary.CFG_Radius = 10
SWEP.Secondary.CFG_RecoilScale = 0.5
SWEP.Secondary.CFG_Sound = "SCW.MM.AR2.Secondary"
SWEP.Secondary.CFG_SoundCharge = "SCW.MM.AR2.Charge"
--
...
if CLIENT then
  local AR2_SELECT_ICON_FONT = "scw_mm_ar2_select_icon"
  local AR2_SELECT_ICON_GLYPH = "l"
  local AR2_SELECT_ICON_COLOR = Color(255, 220, 0)

  surface.CreateFont(AR2_SELECT_ICON_FONT, {
    font = "HalfLife2",
    size = 140,
    weight = 500,
    antialias = true,
  ...
  function SWEP:DrawWeaponSelection(x, y, width, height, alpha)
    AR2_SELECT_ICON_COLOR.a = alpha
    draw.SimpleText(AR2_SELECT_ICON_GLYPH, AR2_SELECT_ICON_FONT, x + width / 2, y + height / 2, AR2_SELECT_ICON_COLOR, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
  end
end
...
function SWEP:_GetPrimaryDamage()
  ...
  if owner:IsNPC() and ConVarExists("sk_npc_dmg_ar2") then return GetConVar("sk_npc_dmg_ar2"):GetInt() end
...
function SWEP:Initialize()
  ...
  self.CFG_PendingSecondary = false
...
function SWEP:GetNPCBulletSpread(proficiency)
  return self.CFG_NPCProficiencySpread[proficiency] or 6
end
...
function SWEP:Precache()
  ...
  util.PrecacheSound(self.Secondary.CFG_SoundCharge)
  util.PrecacheSound("SCW.MM.AR2.Reload_NPC")
...
function SWEP:PrimaryAttack()
  ...
  if self.CFG_PendingSecondary then return end
  ...
  local spread = self.Primary.CFG_NPCSpread
  if owner:IsNPC() then
    ---@cast owner NPC
    local enemy = owner:GetEnemy()
    if IsValid(enemy) then dir = (enemy:WorldSpaceCenter() - owner:GetShootPos()):GetNormalized() end
  elseif self.CFG_ShotsFired <= self.Primary.CFG_SpreadRampStart then
    spread = Vector(self.Primary.CFG_MinSpread, self.Primary.CFG_MinSpread, 0)
  elseif self.CFG_ShotsFired >= self.Primary.CFG_SpreadRampEnd then
    spread = Vector(self.Primary.CFG_MaxSpread, self.Primary.CFG_MaxSpread, 0)
  else
    local frac = (self.CFG_ShotsFired - self.Primary.CFG_SpreadRampStart) / (self.Primary.CFG_SpreadRampEnd - self.Primary.CFG_SpreadRampStart)
    local rampSpread = Lerp(frac, self.Primary.CFG_MinSpread, self.Primary.CFG_MaxSpread)
    spread = Vector(rampSpread, rampSpread, 0)
  end
...
function SWEP:SecondaryAttack()
  ...
  if not IsFirstTimePredicted() or self.CFG_PendingSecondary then return end
  ---@cast owner Player
  ...
  self:EmitSound(self.Secondary.CFG_SoundCharge)
  if not self:_SendSequence("shake") then self:SendWeaponAnim(ACT_VM_FIDGET) end
  self:SetNextPrimaryFire(CurTime() + self.Secondary.CFG_Delay)
  self:SetNextSecondaryFire(CurTime() + self.Secondary.CFG_Delay)
  self.CFG_LastMotionState = "alt"
  self.CFG_PendingSecondary = true
  timer.Simple(self.Secondary.CFG_ChargeDelay, function()
    if not IsValid(self) or not IsValid(owner) then return end
    if SERVER then
      local ball = ents.Create("prop_combine_ball")
      local ang = owner:GetAimVector():Angle()
      local fwd = ang:Forward()
      ball:SetPos(owner:GetShootPos())
      ball:SetAngles(ang)
      ball:SetOwner(owner)
      ball:SetSaveValue("m_flRadius", self.Secondary.CFG_Radius)
      ball:SetSaveValue("m_nMaxBounces", 10)
      ball:Spawn()
      ball:Activate()
      ball:SetSaveValue("m_nState", 2) -- STATE_THROWN
      ball:SetSaveValue("m_flSpeed", self.Secondary.CFG_Force)
      local phys = ball:GetPhysicsObject()
      phys:SetMass(150)
      phys:SetVelocity(fwd * self.Secondary.CFG_Force)
      phys:AddGameFlag(FVPHYSICS_DMG_DISSOLVE)
      ball:Fire("Explode", nil, 2)
    end

    self:EmitSound(self.Secondary.CFG_Sound)
    local sent, duration = self:_SendSequence("fire_alt")
    if not sent then
      self:SendWeaponAnim(ACT_VM_SECONDARYATTACK)
      duration = self:SequenceDuration()
    end

    owner:ViewPunch(Angle(math.Rand(-12, -8), math.Rand(1, 2), 0) * self.Secondary.CFG_RecoilScale)
