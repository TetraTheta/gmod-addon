...
---@param ent Entity
---@return boolean
local function ShouldDisableObstacle(ent)
...
---@param ent Entity
local function ApplyCollision(ent)
  if not cv:GetBool() or not IsValid(ent) then return end
  if ShouldDisableObstacle(ent) then
    ent:SetCollisionGroup(COLLISION_GROUP_WORLD)
  end
end

local function ApplyAllCollisions()
  for _, ent in e_Iterator() do
    ApplyCollision(ent)
  end
end

hook.Add("InitPostEntity", "SCTOOLS_DisableObstacle_Init", ApplyAllCollisions)
hook.Add("OnEntityCreated", "SCTOOLS_DisableObstacle_Create", function(ent)
  if not cv:GetBool() then return end
  timer.Simple(0, function() if IsValid(ent) then ApplyCollision(ent) end end)
end)

cvars.AddChangeCallback("sc_disable_obstacle", function(_, _, new) if tobool(new) then ApplyAllCollisions() end end, "SCTOOLS_DisableObstacle")
